...
# 2. Perform at least two feature engineering steps using recipe() 
...
# Poisson Regression - candidate #4
...
pois_models <- bike_pois_workflow %>% 
  tune_grid(resamples = folds, 
            metric = metric,
            control = tunedModel)
...
vroom_write(stacked_preds, "FinalPreds.csv", delim = ",")


# FINAL PREDICTIONS - so far, my random forest has gotten me the lowest score.. we're sticking w it! 
...
more_preds$datetime <- as.character(format(bikeTest$datetime))
names(more_preds)[1] <- "count"
more_preds$count <- exp(more_preds$count)
vroom_write(more_preds, "FinalPreds.csv", delim = ",")
